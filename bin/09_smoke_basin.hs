...
import Data.Array.Unboxed ((!), UArray, assocs, bounds, inRange, listArray)
...
lows hgts = filter low (assocs hgts)
  where low ((y, x), v) = all (ok v) [(y - 1, x), (y, x - 1), (y, x + 1), (y + 1, x)]
        ok h npos = not (bounds hgts `inRange` npos) || h < hgts ! npos
...
neigh hgts (y, x) (dy, dx) | bounds hgts `inRange` npos = let v = hgts ! npos in if v == 9 then Nothing else Just npos
